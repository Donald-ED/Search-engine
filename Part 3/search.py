...
def title_to_info(metadata):
  ...
  for i in range(len(metadata)):
    dict_info[metadata[i][0]] = {'author':metadata[i][1], 'timestamp':metadata[i][2], 'length':metadata[i][3]}
  return dict_info  
...
def keyword_to_titles(metadata):
  ...
  for data in metadata:
    for word in data[4]:
      if word in dict_keyword_titles:
        dict_keyword_titles[word].append(data[0])
      else:
        dict_keyword_titles[word] = [data[0]]  
  return dict_keyword_titles       
...
def article_info(titles, title_to_info):
  dict_titles = {}
  for items in titles:
    dict_titles[items] = title_to_info[items]
  return dict_titles   
...
def article_length(article_length, titles, title_to_info):
  dict_items = article_info(titles, title_to_info)
  for keys in dict_items:
    if dict_items[keys]["length"] > article_length:
      titles.remove(keys)
  return titles    
...
def title_timestamp(titles, title_to_info):
  ...
  for items in dict_items:
    new_dict[items] = dict_items[items]["timestamp"]
  return new_dict
...
def favorite_author(author, titles, title_to_info):
  dict_items = article_info(titles, title_to_info)
  for items in dict_items:
    if dict_items[items]["author"].lower().replace(" ", "") == author.lower().replace(" ", ""):
      return True
  return False    
